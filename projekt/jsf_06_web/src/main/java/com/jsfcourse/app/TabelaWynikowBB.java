...
import com.jsf.dao.TabelaWynikowDAO;
import com.jsf.entities.Uzytkownik;

@Named
@RequestScoped
public class TabelaWynikowBB {
	...
	public List<Uzytkownik> getList(){
		...
		//2. Get list
		list = uzytkDAO.getList(searchParams);
	...
	public String editPerson(Uzytkownik person){
		//1. Pass object through session
		//HttpSession session = (HttpSession) extcontext.getSession(true);
		//session.setAttribute("person", person);
	...
	public String deletePerson(Uzytkownik person){
		uzytkDAO.remove(person);
